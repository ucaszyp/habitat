...
def main():
    ...
    # tensorboard
    # if not args.eval:
    #     train_writer = SummaryWriter(args.dump_location)
    # if args.eval:
    #     test_writer = SummaryWriter(args.dump_location)
    ...
    #obs: tensor [25, 20, 120, 160]
    #info: tuple, len = num_scenes. 
    #element of info: dict{'distance_to_goal', 'spl', 'success', 'time', 'sensor_pose', 'goal_cat_id', 'goal_name'}
    ...
    full_map = torch.zeros(num_scenes, nc, full_w, full_h).float().to(device)  # torch ([25, 20, 480, 480])
    local_map = torch.zeros(num_scenes, nc, local_w, local_h).float().to(device) # torch ([25, 20, 240, 240])

    # Initial full and local pose
    full_pose = torch.zeros(num_scenes, 3).float().to(device) # torch ([25, 3])
    local_pose = torch.zeros(num_scenes, 3).float().to(device) # torch ([25, 3])

    # Inintial angle map
    full_angle_map = torch.zeros(num_scenes, 12, nc, full_w, full_h).float().to(device) # torch ([25, 12, 20, 480, 480])
    local_angle_map = torch.zeros(num_scenes, 12, nc, local_w, local_h).float().to(device) # torch ([25, 12, 20, 240, 240])
    
    # Inintial local angle
    # local_angle = 

    # Origin of local map
    origins = np.zeros((num_scenes, 3)) # numpy (25, 3)

    # Local Map Boundaries
    lmb = np.zeros((num_scenes, 4)).astype(int) # numpy (25, 4)
    ...
    def init_map_and_pose():
        # init full map, pose, angle_map
        full_map.fill_(0.)
        full_pose.fill_(0.)
        full_angle_map.fill_(0.)

        # init to center of the map
        full_pose[:, :2] = args.map_size_cm / 100.0 / 2.0

        locs = full_pose.cpu().numpy()
        planner_pose_inputs[:, :3] = locs # numpy (25, 7)
        for e in range(num_scenes):
            r, c = locs[e, 1], locs[e, 0] # (r, c) means (x, y) in meter
            # loc_r, loc_c transform meter into map unit (5cm * 5cm)
            loc_r, loc_c = [int(r * 100.0 / args.map_resolution),
                            int(c * 100.0 / args.map_resolution)]
            
            full_map[e, 2:4, loc_r - 1:loc_r + 2, loc_c - 1:loc_c + 2] = 1.0
            full_angle_map[e, 0, 2:4, loc_r - 1:loc_r + 2, loc_c - 1:loc_c + 2] = 1.0
        ...
        for e in range(num_scenes):
            ...
            # add local angle map
            local_angle = int(local_pose[e, 2] / 30)
            local_angle_map[e, local_angle] = local_map[e]

    def init_map_and_pose_for_env(e):
        # init full map, pose, angle_map
        full_map[e].fill_(0.)
        full_pose[e].fill_(0.)
        full_angle_map[e].fill_(0.)
    ...
    def update_intrinsic_rew(e):
        ...
        prev_explored_angle = full_angle_map[e, :,1].sum(2).sum(1).sum(0)
        full_angle_map[e, :, :, lmb[e, 0]:lmb[e, 1], lmb[e, 2]:lmb[e, 3]] = local_angle_map[e]
        curr_explored_angle = full_angle_map[e, :, 1].sum(2).sum(1).sum(0)

        intrinsic_rew = curr_explored_area - prev_explored_area
        angle_rew = curr_explored_angle - prev_explored_angle
        
        intrinsic_rews[e] = 0.9 * intrinsic_rew + 0.1 * angle_rew
    ...
    # Global policy g_observation_space.shape base_kwargs
    ...
    for e in range(num_scenes):
        ...
        # update local_angle_map
        local_angle = int(local_pose[e, 2] / 30)
        local_angle_map[e, local_angle, 2:4, loc_r - 1:loc_r + 2, loc_c - 1:loc_c + 2] = 1.
    ...
    for e, p_input in enumerate(planner_inputs):
        ...
        if args.get_bird == 1:
            p_input['full'] = full_map[e, :, :, :].cpu().numpy()
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            step_index = step * num_scenes
            reward_str = str(step_index) + ": "
            for e in range(num_scenes):
                reward_str = reward_str + " " + str(intrinsic_rews[e])
            with open('intrinsic_reward.txt', "a") as f:
        ...
        if step % args.log_interval == 0:
            ...
            if len(g_episode_rewards) > 0:
                ...
                # train_writer.add_scalar("GSR-mean", np.mean(per_step_g_rewards), step)
                # train_writer.add_scalar("GSR-median", np.median(per_step_g_rewards), step)
                # train_writer.add_scalar("GER-mean", np.mean(g_episode_rewards), step)
                # train_writer.add_scalar("GER-median", np.median(g_episode_rewards), step)
                # train_writer.add_scalar("GER-min", np.min(g_episode_rewards), step)
                # train_writer.add_scalar("GER-max", np.max(g_episode_rewards), step)
            ...
            if len(g_value_losses) > 0 and not args.eval:
                ...
                # train_writer.add_scalar("Policy Loss value", np.mean(g_value_losses), step)
                # train_writer.add_scalar("Policy Loss action", np.mean(g_action_losses), step)
                # train_writer.add_scalar("Policy Loss dist", np.mean(g_dist_entropies), step)

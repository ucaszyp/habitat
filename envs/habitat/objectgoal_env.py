...
class ObjectGoal_Env(habitat.RLEnv):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        self.episodes_dir = config_env.DATASET.EPISODES_DIR.format(
            split=self.split)
        if "objectnav_gibson" in args.task_config:
    ...
    def reset(self):
        ...
        if new_scene:
            obs = super().reset()
            self.scene_name = self.habitat_env._config.SIMULATOR.SCENE 
            print("Changing scene: {}/{}".format(self.rank, self.scene_name))

        self.scene_path = self.habitat_env._config.SIMULATOR.SCENE 
        if "objectnav_gibson" in args.task_config:
            if self.split == "val":

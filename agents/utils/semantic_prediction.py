...
from constants import coco_categories_mapping, nyu_category
from Cerberus.predictor import Cerberus_Seg


class SemanticPredMaskRCNN():

    def __init__(self, args, rank):
        self.segmentation_model = ImageSegmentation(args, rank)
        self.args = args
        self.nyu_dict = nyu_category
    ...
    def get_prediction(self, img):
        ...
        seg_predictions = self.segmentation_model.get_predictions(img)
        semantic_input = np.zeros((img.shape[0], img.shape[1], args.classes + 1))
        for i in range(args.classes):
            object_mask = (seg_predictions == i)
            object_idx = self.nyu_dict[i]
            semantic_input[:, :, object_idx] = object_mask.astype(np.uint8)
